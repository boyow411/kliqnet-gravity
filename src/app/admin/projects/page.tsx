...
export default function AdminProjectsPage() {
  ...
  /* eslint-disable react-hooks/exhaustive-deps -- admin table fetches after pagination/filter changes; search is submitted explicitly. */
  useEffect(() => {
  ...
  async function handleDelete(id: number) {
    if (
      !confirm(
        "Unpublish this project? It will remain available in the editor.",
      )
    )
      return;
                ...
                >
                  ...
                  <td className="px-5 py-3 hidden md:table-cell">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-md font-medium border ${
                        project.category === "Venture Studio"
                  ...
                  <td className="px-5 py-3 text-sm text-gray-400 hidden sm:table-cell">
                    <span className="inline-flex items-center gap-1.5">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${project.status === "Live" ? "bg-green-400" : "bg-yellow-400"}`}
                      />
                      {project.status}
                    </span>
                  </td>
